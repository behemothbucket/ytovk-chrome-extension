...
buttonDeleteAll.addEventListener("click", () => {
	...
	document.querySelector(".track_wrapper")?.remove();
	if (!document.querySelector(".empty_downloads_wrapper")) showEmptyDownloadsText();
});
...
function showEmptyDownloadsText() {
	...
	emptyDownloadsTextDiv.innerHTML = "<span class='empty_downloads_text'>No downloads</span>";
...
function renderTracks(downloads) {
	...
	`;
			...
			document.querySelector(".button_download").addEventListener("click", downloadTrack);
		}
	}
}
...
function downloadTrack(event) {
	...
	let artist = event.target.parentNode.querySelector(".track_info").querySelector(".artist").textContent;
	let title = event.target.parentNode.querySelector(".track_info").querySelector(".title").textContent;
	let filename = `${artist} - ${title}.mp3`.replace(/[`~!@#$%^&*()_|+=?;:'",.<>{}[\]\\/]/gi, "");
